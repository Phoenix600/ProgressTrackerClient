...
exports.createJavaCourse = createJavaCourse;
const java_course_data_1 = require("../data/java-course-data");
...
async function createJavaCourse() {
    try {
        ...
        console.log(`📚 Course: ${java_course_data_1.javaCourseData.name}`);
        console.log(`📖 Chapters: ${java_course_data_1.javaCourseData.chapters.length}`);
        console.log(`🎯 Total Topics: ${java_course_data_1.javaCourseData.chapters.reduce((acc, chap) => acc + chap.topics.length, 0)}`);
        console.log(`⏱️  Total Duration: ${java_course_data_1.javaCourseData.chapters.reduce((acc, chap) => acc + chap.durationDays, 0)} days`);
        console.log('');
        const response = await fetch(`${API_BASE_URL}/courses`, {
            ...
            body: JSON.stringify(java_course_data_1.javaCourseData),
        });
        if (!response.ok) {
